			...
			<thead>
				...
				<g:each in="${inscriptionInstanceList}" status="i" var="inscriptionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${inscriptionInstance.id}">${fieldValue(bean: inscriptionInstance, field: "dateInscription")}</g:link></td>
					
						<td>${fieldValue(bean: inscriptionInstance, field: "activite")}</td>
