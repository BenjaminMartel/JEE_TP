...
class JeuTestService {
    ...
    def createJeuTestFoActivite() {
        if(Activite.count() == 0) {
            Activite activite1 = new Activite(titre: "activite1")
            Activite activite2 = new Activite(titre: "activite2")
            Activite activite3 = new Activite(titre: "activite3")

            Utilisateur unResponsable = Utilisateur.findById(1)

            activiteService.insertOrUpdateActiviteForResponsable(activite1, unResponsable)
            activiteService.insertOrUpdateActiviteForResponsable(activite2, unResponsable)
            //RuntimeException()
            activiteService.insertOrUpdateActiviteForResponsable(activite3, unResponsable)
        }
    }
}*/
